...
export async function POST(request) {
  try {
    ...
    const { name, type, version } = await request.json();
    ...
    // Determine endpoint based on database type (Coolify has specific endpoints for postgresql, mysql, etc)
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to create database in Coolify" }, { status: 500 });
    }
